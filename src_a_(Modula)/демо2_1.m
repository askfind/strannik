...
procedure ƒобавитьЁлемент¬ƒерево(окно:HWND; ниже,после:HTREEITEM; текст:pstr, иконка,иконка¬ыдел:HBITMAP):HTREEITEM;
...
begin
  ...
  return HTREEITEM(SendMessage(окно,TVM_INSERTITEM,0,integer(addr(структ))))
end ƒобавитьЁлемент¬ƒерево;

//================= создать окно с деревом =======================

procedure —оздатьƒерево(главное:HWND; ид:cardinal);
...
procedure ќбработать»звещениеќтƒерева(окно:HWND; инфо:pTV_DISPINFO):boolean;
begin
  ...
  return false
end ќбработать»звещениеќтƒерева;
...
begin
  case message of
    WM_INITDIALOG:—оздатьƒерево(wnd,идќкноƒерево); «аполнитьƒерево(GetDlgItem(wnd,идќкноƒерево));|
    ...
    WM_NOTIFY:case loword(wparam) of
      идќкноƒерево:return ќбработать»звещениеќтƒерева(GetDlgItem(wnd,идќкноƒерево),address(lparam));|
    end;|
  else return false
  end;
  return true
