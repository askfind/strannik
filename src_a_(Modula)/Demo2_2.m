...
procedure id8(id9:HWND; id10:cardinal);
...
procedure id25(id16:HWND; id26:pLV_DISPINFO):boolean;
begin
  with id26^.hdr,id26^.item do
  case code of
    LVN_GETDISPINFO:if mask and LVIF_TEXT<>0 then //text
...
begin
  case message of
    WM_INITDIALOG:id8(wnd,id5); id19(GetDlgItem(wnd,id5));|
    ...
    WM_NOTIFY:case loword(wparam) of
      id5:return id25(GetDlgItem(wnd,id5),address(lparam));|
    end;|
  else return false
  end;
  return true
