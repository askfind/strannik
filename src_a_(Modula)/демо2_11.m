...
procedure procDLG_CAL(wnd:HWND; message,wparam,lparam:integer):boolean;
begin
  case message of
    WM_INITDIALOG:SendDlgItemMessage(wnd,100,MCM_SETCURSEL,0,integer(addr(дата)));|
    WM_COMMAND:case loword(wparam) of
      IDOK,IDCANCEL:SendDlgItemMessage(wnd,100,MCM_GETCURSEL,0,integer(addr(дата))); EndDialog(wnd,1);|
    end;|
  else return false
  end;
  return true
