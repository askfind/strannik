...
begin
  ...
  with элемент do begin
    ...
    SendMessage(заголовок,HDM_INSERTITEM,тек,dword(addr(элемент)));
...
with пар^ do begin
  ...
  тек:=0;
  смещ:=0;
  while lstrposc('\9',текст)>=0 do begin
    lstrcpy(строка,текст);
    строка[lstrposc('\9',текст)]:='\0';
    элем.mask:=HDI_WIDTH;
...
begin
  case message of
    ...
    WM_DRAWITEM:if wparam<>0 then НарисоватьСтроку(GetDlgItem(wnd,101),pDRAWITEMSTRUCT(lparam));
