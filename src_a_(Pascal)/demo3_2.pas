...
function sqlCALL(conn:HANDLE; fun:pstr; ret:integer):integer;
...
//insert records
  sqlEXECUTE(connection,"INSERT INTO Personal (PersId,PersName) VALUES (0001,'James');");
  sqlEXECUTE(connection,"INSERT INTO Personal (PersId,PersName) VALUES (0002,'Andrew');");
  sqlEXECUTE(connection,"INSERT INTO Personal (PersId,PersName) VALUES (0003,'Sam');");
//output table
  ...
  while SQLFetch(stat)=SQL_SUCCESS do begin
    SQLGetData(stat,1,SQL_C_SLONG,addr(id),0,nil); mbI(id,"id");
    SQLGetData(stat,2,SQL_C_CHAR,addr(name),1000,nil); MessageBox(0,name,"name",0);
  end;
