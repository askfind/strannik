...
function procDLG_1(wnd:HWND; message,wparam,lparam:integer):boolean;
var сооб:pNMHDR;
begin
  case message of
    WM_INITDIALOG:;
    ...
    case сооб^.code of
      ...
      PSN_KILLACTIVE:begin
        if IsDlgButtonChecked(wnd,идПереключ0)=BST_CHECKED then вариант:=0;
        if IsDlgButtonChecked(wnd,идПереключ1)=BST_CHECKED then вариант:=1;
        if IsDlgButtonChecked(wnd,идПереключ2)=BST_CHECKED then вариант:=2;
      end;
    end;
    procDLG_1:=false;
    end;
  else procDLG_1:=false
  end;
  procDLG_1:=true
end;

function procDLG_2(wnd:HWND; message,wparam,lparam:integer):boolean;
var сооб:pNMHDR;
begin
  case message of
    WM_INITDIALOG:;
...
//инициализация страниц
  ...
  with страницы[текущая] do begin
