...
begin
  ...
  id74:=0;
  for id73:=id2 to id7 do begin
    id75:=(id72.right-id72.left+1)*id9[id73] div 100;
    id71[id73]:=id74+id75;
    inc(id74,id75)
  end;
  id71[id7]:=-1;
  SendMessage(id12,SB_SETPARTS,ord(id7)+1,dword(addr(id71)));
end;
...
begin
  SendMessage(id12,SB_SETTEXT,ord(id2),dword(addr(id15)));
  if boolean(SendMessage(id11,EM_GETMODIFY,0,0))
    then SendMessage(id12,SB_SETTEXT,ord(id3),dword("Изменен"))
    else SendMessage(id12,SB_SETTEXT,ord(id3),dword(""));
  with id78 do begin
    cbSize:=sizeof(CHARFORMAT);
    dwMask:=CFM_FACE | CFM_SIZE | CFM_BOLD | CFM_ITALIC | CFM_UNDERLINE;
    SendMessage(id11,EM_GETCHARFORMAT,1,dword(addr(id78)));
    SendMessage(id12,SB_SETTEXT,ord(id4),dword(addr(szFaceName)));
    yHeight:=yHeight div 20;
    wvsprintf(id77,"Размер:%li",addr(yHeight));
    SendMessage(id12,SB_SETTEXT,ord(id5),dword(addr(id77)));
    if dwEffects and CFE_BOLD=0
      then SendMessage(id12,SB_SETTEXT,ord(id6),dword(""))
      else SendMessage(id12,SB_SETTEXT,ord(id6),dword("Полужирный"));
    if dwEffects and CFE_ITALIC=0
      then SendMessage(id12,SB_SETTEXT,ord(id7),dword(""))
      else SendMessage(id12,SB_SETTEXT,ord(id7),dword("Курсив"));
  end
end;
...
begin
  with id78 do begin
    ...
    SendMessage(id11,EM_SETPARAFORMAT,0,dword(addr(id78)));
  end
end;
...
begin
  with id78 do begin
    ...
    SendMessage(id11,EM_SETCHARFORMAT,SCF_SELECTION,dword(addr(id78)));
  end
end;

//-------------------- test text -------------------------

function id83(id84:pstr):boolean;
var id85:integer;
begin
  id85:=lstrlen(id84)-4;
...
function id86(dwCookie:dword; pbBuff:address; cb:dword; pcb:pINT):dword;
begin
...
begin
  with id89 do begin
    id88:=_lopen(id84,0);
    if id88>0 then begin
      dwCookie:=id88;
      dwError:=0;
      pfnCallback:=addr(id86);
      if id83(id84)
        then SendMessage(id11,EM_STREAMIN,SF_TEXT,dword(addr(id89)))
        else SendMessage(id11,EM_STREAMIN,SF_RTF,dword(addr(id89)));
      _lclose(id88);
...
function id90(dwCookie:dword; pbBuff:address; cb:dword; pcb:pINT):dword;
begin
...
begin
  with id89 do begin
    ...
    if id88>0 then begin
      ...
      if id83(id84)
        then SendMessage(id11,EM_STREAMOUT,SF_TEXT,dword(addr(id89)))
        else SendMessage(id11,EM_STREAMOUT,SF_RTF,dword(addr(id89)));
      _lclose(id88);
      SendMessage(id11,EM_SETMODIFY,0,0)
    end
  end
...
function id93(wnd:HWND; msg,wparam,lparam:dword):dword;
begin
  case msg of
...
begin
  ...
  with id60[id97] do begin
    //group
    for id98:=id57 to id58 do
    if (id62[id98]>0)and(id96<50) then begin
      inc(id96);
      RtlZeroMemory(addr(id95[id96]),sizeof(TBBUTTON));
      with id95[id96] do begin
        iBitmap:=id62[id98]-1;
...
begin
  if not(not id83(id15)and
    (MessageBox(id10,"You are sure ?",nil,MB_YESNO)<>IDYES)) then begin
...
begin
  with id72 do begin
    cpMin:=0;
    cpMax:=-1;
    SendMessage(id11,EM_EXSETSEL,0,dword(addr(id72)));
  end
end;
...
begin
  if (id117 and
    ...
    with id118 do begin
      ...
      chrg.cpMax:=-1;
      lpstrText:=addr(id16);
      id119:=SendMessage(id11,EM_FINDTEXTEX,0,dword(addr(id118)));
      if id119=-1
        then MessageBox(id11,"Фрагмент не найден","ВНИМАНИЕ:",MB_ICONSTOP)
...
begin
  ...
  //select font
    if ChooseFont(id121) then begin
  //fill symbol format
      with id122 do begin
...
begin
  ...
  //print document
    ...
    if id126<0 then MessageBox(id11,"Ошибка печати","ВНИМАНИЕ:",MB_ICONSTOP)
    else begin
      id127:=0;
      id128:=SendMessage(id11,WM_GETTEXTLENGTH,0,0);
      while id127<id128 do begin
      //format and print
        id127:=SendMessage(id11,EM_FORMATRANGE,1,dword(addr(id78)));
        if id127<id128 then begin
          EndPage(dc);
          StartPage(dc);
...
function id129(wnd:HWND):boolean;
var id130:dword;
begin
   if boolean(SendMessage(id11,EM_GETMODIFY,0,0)) then begin
     id130:=MessageBox(id11,"File was modified. Save ?",nil,MB_ICONSTOP | MB_YESNOCANCEL);
     case id130 of
       IDYES:begin id91(wnd,id15); return true end;
       IDNO:return true;
...
begin
  ...
  with id60[id97] do begin
    ...
    for id98:=id57 to id58 do begin
      if id59[id98][0]='\0'
...
function id134(code,wparam,lparam:integer):dword;
var id135:pMSG;
begin
  id135:=pMSG(lparam);
  with id135^ do
...
function id136(wnd:HWND; msg,wparam,lparam:dword):dword;
begin
  case msg of
    WM_INITDIALOG:begin
      id131(wnd);
    ...
    WM_SIZE:begin id102(wnd); id68(wnd,true) end;
    WM_COMMAND:case loword(wparam) of
      id63+id18:if id129(wnd) then SetWindowText(id11,"");
      id63+id19:if id129(wnd) then id110(wnd);
      id63+id20:id113(wnd);
      id63+id21:id114(wnd);
      id63+id23:id123(wnd);
      ...
      IDCANCEL,id63+id25,id63+id46:if id129(wnd) then begin
