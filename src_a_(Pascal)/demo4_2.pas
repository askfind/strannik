...
//init WinInet and session
  ...
  if internet=0 then MessageBox(0,"InternetOpen error",nil,0);
  session:=InternetConnect(internet,"home.perm.ru",INTERNET_DEFAULT_FTP_PORT,"strannik","",INTERNET_SERVICE_FTP,0,0);
  if session=0 then MessageBox(0,"InternetConnect error",nil,0);
//find files
  buffer[0]:='\0';
  ...
  if (find=0)and(GetLastError()<>ERROR_NO_MORE_FILES) then MessageBox(0,"FtpFindFirstFile error",nil,0)
    ...
    if GetLastError()<>ERROR_NO_MORE_FILES then MessageBox(0,"InternetFindNextFile error",nil,0);
  end;
