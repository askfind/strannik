...
//read file from url
function ReadFileByUrl(url:pstr; text:pstr; max:integer):integer;
var
  internet:HINTERNET;
  file:HINTERNET;
  number:integer;
  ...
  result:boolean;
begin
  internet:=InternetOpen("Strannik",INTERNET_OPEN_TYPE_PRECONFIG,nil,nil,0); if internet=0 then return 1;
  file:= InternetOpenUrl(internet,url,nil,0,0,0); if file=0 then begin InternetCloseHandle(internet); return 2 end;
  size:=0;
  repeat
    if size+500>max
      then read:=max-size
...
begin
  case message of
    WM_INITDIALOG:begin
      ...
      SendDlgItemMessage(wnd,idHtml,EM_EXLIMITTEXT,0,maxFile) end;
