...
function „итать‘айлѕоUrl(url:pstr; текст:pstr; макс:integer):integer;
var
  интернет:HINTERNET;
  файл:HINTERNET;
  количество:integer;
  ...
  результат:boolean;
begin
  интернет:=InternetOpen("Strannik",INTERNET_OPEN_TYPE_PRECONFIG,nil,nil,0); if интернет=0 then return 1;
  файл:= InternetOpenUrl(интернет,url,nil,0,0,0); if файл=0 then begin InternetCloseHandle(интернет); return 2 end;
  размер:=0;
  repeat
    if размер+500>макс
...
begin
  case message of
    WM_INITDIALOG:begin
      ...
      SendDlgItemMessage(wnd,idHtml,EM_EXLIMITTEXT,0,макс‘айл) end;
